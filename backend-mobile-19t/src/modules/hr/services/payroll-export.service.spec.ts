...
describe('PayrollExportService', () => {
  ...
  beforeEach(async () => {
    ...
    configService = {
      get: jest.fn((key: string, fallback: string) => {
        if (key === 'ODOO_SERVICE_USERNAME') return 'service@example.com';
        if (key === 'ODOO_SERVICE_PASSWORD') return 'secret';
        return fallback;
      }),
    };
  ...
  it('builds an xlsx workbook for active employees and payroll cycle boundaries', async () => {
    ...
    expect(worksheet?.getCell('F2').value).toBe(2);
    expect(worksheet?.getCell('G2').value).toBe(4);
    expect(worksheet?.getCell('H2').value).toBe(2);
  ...
  it('rejects an employee payroll summary for non-approvers', async () => {
    await expect(
      service.getEmployeePayrollSummary(
        'requester-1',
        ['employee'],
        '2026-08',
        'employee-1',
  ...
  it('rejects an unknown employee payroll summary', async () => {
    ...
    ).rejects.toThrow('Nhân viên không tồn tại');
  });

  it('reports unmapped attendance separately from zero days', async () => {
    ...
    configRepo.findOne.mockResolvedValue({ payroll_start_day: 25 });

    const result = await service.getEmployeePayrollSummary(
      'requester-1',
      ['manager'],
      '2026-08',
      'employee-1',
    ...
    expect(odooService.fetchAttendanceHistory).not.toHaveBeenCalled();
  });

  it('reports unavailable attendance when Odoo fails', async () => {
    userRepo.findOne.mockResolvedValue({
      id: 'employee-1',
      is_bot: false,
      odoo_uid: 100,
    ...
    configRepo.findOne.mockResolvedValue({ payroll_start_day: 25 });
    odooService.fetchAttendanceHistory.mockRejectedValue(
      new Error('Odoo unavailable'),
    );
    ...
    expect(result.attendance_status).toBe('unavailable');
    expect(result.actual_working_days).toBeNull();
  });

  it('reports unavailable attendance when Odoo credentials are missing', async () => {
    userRepo.findOne.mockResolvedValue({
      id: 'employee-1',
      is_bot: false,
      odoo_uid: 100,
  ...
  it('reports unavailable attendance when Odoo mapping resolution fails', async () => {
    userRepo.findOne.mockResolvedValue({
      id: 'employee-1',
      is_bot: false,
      odoo_uid: 100,
    ...
    configRepo.findOne.mockResolvedValue({ payroll_start_day: 25 });
    odooService.findEmployeeIdByUserUidOrEmployeeId.mockRejectedValue(
      new Error('Odoo mapping unavailable'),
    );
  ...
  it('returns a genuine zero when Odoo succeeds with no completed sessions', async () => {
    userRepo.findOne.mockResolvedValue({
      id: 'employee-1',
      is_bot: false,
      odoo_uid: 100,
    ...
    expect(result.attendance_status).toBe('available');
    expect(result.actual_working_days).toBe(0);
  });

  it('uses half-open payroll boundaries and counts completed dates', async () => {
    userRepo.findOne.mockResolvedValue({
      id: 'employee-1',
      is_bot: false,
      odoo_uid: 100,
    ...
    odooService.fetchAttendanceHistory.mockResolvedValue([
      {
        ...
        check_in: '2026-07-25 01:00:00',
        check_out: '2026-07-25 10:00:00',
      },
      {
        id: 2,
        employee_id: [200, 'Employee'],
        check_in: '2026-08-24 01:00:00',
        check_out: '2026-08-24 10:00:00',
      },
    ]);
    ...
    expect(result.actual_working_days).toBe(1.5);
  });

  it('returns attendance audit sessions and overlapping employee orders', async () => {
    userRepo.findOne.mockResolvedValue({
      id: 'employee-1',
      is_bot: false,
      odoo_uid: 100,
    ...
    odooService.fetchAttendanceHistory.mockResolvedValue([
      {
        id: 1,
        employee_id: [200, 'Employee'],
        check_in: '2026-08-08 01:00:00',
        check_out: '2026-08-08 10:00:00',
        worked_hours: 9,
      },
      {
        id: 2,
        employee_id: [200, 'Employee'],
        check_in: '2026-08-08 11:00:00',
        check_out: false,
      },
      {
        id: 3,
        employee_id: [200, 'Employee'],
        check_in: '2026-08-08 15:00:00',
        check_out: '2026-08-08 18:00:00',
      },
    ]);
    ...
    expect(result.actual_working_days).toBe(0.5);
    expect(result.attendance_sessions).toEqual([
      expect.objectContaining({
        id: 1,
        date: '2026-08-08',
      ...
      expect.objectContaining({
        ...
        date: '2026-08-08',
        counted: false,
        exclusion_reason: 'missing_checkout',
      }),
      expect.objectContaining({
        id: 3,
        date: '2026-08-08',
        counted: false,
        exclusion_reason: 'overnight',
      }),
    ]);
    expect(result.leave_orders).toEqual([
      expect.objectContaining({
        id: 'leave-1',
        type: 'annual',
        requested_days: 0.5,
  ...
  it('derives January payroll boundaries across the year rollover', async () => {
    userRepo.findOne.mockResolvedValue({
      id: 'employee-1',
      is_bot: false,
      odoo_uid: 100,
    ...
    expect(result.cycle_from).toBe('2026-12-25');
    expect(result.cycle_to_exclusive).toBe('2027-01-25');
  });
});
