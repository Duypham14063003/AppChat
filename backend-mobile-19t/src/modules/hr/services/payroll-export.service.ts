...
export type EmployeePayrollSummary = {
  ...
  month: string;
  cycle_from: string;
  cycle_to_exclusive: string;
  attendance_status: 'available' | 'unmapped' | 'unavailable';
  actual_working_days: number | null;
  attendance_sessions: EmployeePayrollAttendanceSession[];
  leave_orders: EmployeePayrollLeaveOrder[];
};

export type EmployeePayrollAttendanceSession = {
  id: number;
  date: string | null;
...
export type EmployeePayrollLeaveOrder = {
  id: string;
  user_id: string;
  type: string;
  start_date: string;
...
export class PayrollExportService {
  ...
  async getEmployeePayrollSummary(
    ...
    if (!canViewEmployeeAttendanceRoles(roles)) {
      throw new ForbiddenException('Không có quyền xem ngày công nhân viên');
    }

    const user = await this.userRepo.findOne({
      where: { id: targetUserId, is_bot: false },
    });
    if (!user) {
    ...
    const base = {
      user_id: user.id,
      month,
      cycle_from: this.toDateKey(cycle.from),
      cycle_to_exclusive: this.toDateKey(this.makeExclusiveEnd(cycle.to)),
      attendance_sessions: [] as EmployeePayrollAttendanceSession[],
      leave_orders: await this.loadEmployeeLeaveOrders(user.id, cycle),
    };

    if (!user.odoo_employee_id && !user.odoo_uid) {
      return {
        ...base,
    ...
    try {
      this.assertOdooAttendanceConfigured();
      ...
      const records = await this.odooService.fetchAttendanceHistory(
        employeeId,
        this.toHoChiMinhUtcBoundary(cycle.from),
        this.toHoChiMinhUtcBoundary(this.makeExclusiveEnd(cycle.to)),
      );
      return {
        ...base,
        attendance_status: 'available',
        actual_working_days:
          calculateCompletedAttendanceDays(records).totalDays,
        attendance_sessions: records.map((record) =>
          this.buildAttendanceSessionDetail(record),
        ),
      };
    } catch (error) {
      this.logger.warn(
        `Unable to load payroll attendance for user=${user.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return {
        ...base,
  ...
  private async loadEmployeeLeaveOrders(
    userId: string,
    cycle: PayrollCycleRange,
  ): Promise<EmployeePayrollLeaveOrder[]> {
    const cycleFrom = this.toDateKey(cycle.from);
    const cycleToExclusive = this.toDateKey(this.makeExclusiveEnd(cycle.to));
    const leaves = await this.leaveRepo.find({
      where: {
        user_id: userId,
        start_date: LessThan(cycleToExclusive),
        end_date: MoreThanOrEqual(cycleFrom),
      },
    ...
    return leaves.map((leave) => ({
      ...
      requested_days: Number(leave.requested_days) || 0,
      paid_days: Number(leave.paid_days) || 0,
      unpaid_days: Number(leave.unpaid_days) || 0,
      ot_time: Number(leave.ot_time) || 0,
      reason: leave.reason,
      status: leave.status,
    }));
  }

  private buildAttendanceSessionDetail(
    ...
    return {
      ...
      check_in: evaluation.checkin?.toISOString() ?? null,
      check_out: evaluation.checkout?.toISOString() ?? null,
      worked_hours:
        typeof record.worked_hours === 'number' ? record.worked_hours : null,
      counted: evaluation.counted,
      day_value:
        evaluation.counted && evaluation.dateKey
          ? getCompletedAttendanceDayValue(evaluation.dateKey)
          : null,
      exclusion_reason: evaluation.exclusionReason,
    };
  }

  private assertOdooAttendanceConfigured(): void {
    ...
    if (!username || !password) {
      throw new Error('Odoo service account credentials not configured');
    }
  }

  private buildPayrollCycle(
    month: string,
    startDay: number,
  ): PayrollCycleRange {
  ...
  private async computePayrollAggregates(
    ...
    const attendanceDays = new Set(
      attendanceRecords.flatMap((record) => {
        ...
        return checkin ? [toHoChiMinhDateKey(checkin)] : [];
      }),
    );
    const actualWorkedDays =
      calculateCompletedAttendanceDays(attendanceRecords).totalDays;
    ...
    for (const workday of this.getWorkdayDates(cycle.from, cycle.to)) {
      ...
      if (attendanceDays.has(workday)) {
        payrollPresenceDays += 1;
        continue;
      }

      payrollPresenceDays += wfh;
    ...
    payrollPresenceDays = this.roundDays(payrollPresenceDays);
    ...
    return {
      ...
      actualWorkedDays,
      payrollWorkedDays: this.roundDays(payrollPresenceDays + paidLeaveDays),
      wfhDays,
  ...
  private toHoChiMinhUtcBoundary(localDate: Date): Date {
    const boundary = this.toStartOfUtcDay(localDate);
    boundary.setUTCHours(boundary.getUTCHours() - 7);
    return boundary;
  }
