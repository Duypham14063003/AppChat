...
export class AuthController {
  ...
  @ApiBearerAuth()
  @Patch('sessions/voip-token')
  @ApiOperation({ summary: 'Update VoIP token for current session' })
  async updateVoipToken(
    @CurrentUser('userId') userId: string,
    @Body() body: { device_id: string; voip_token: string },
  ) {
    const session = await this.sessionService.findSessionByUserAndDevice(
      userId,
      body.device_id,
    ...
    return { message: 'VoIP token updated' };
  }
}
